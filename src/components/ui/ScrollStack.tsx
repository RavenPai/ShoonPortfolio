...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const getScrollData = useCallback(() => {
    if (useWindowScroll) {
      ...
      return {
        ...
        containerHeight: containerHeightRef.current || (scroller ? scroller.clientHeight : 0),
  ...
  const updateCardOffsets = useCallback(() => {
    ...
    if (useWindowScroll) {
      ...
      containerHeightRef.current = scrollerRef.current.clientHeight;
    }

    // Update end element top
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // Use cached offsets if available, otherwise calculate
    if (cardOffsetsRef.current.length !== cardsRef.current.length) {
      updateCardOffsets();
    }
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (hasChanged) {
        ...
        // Ensure proper overlap: current top card should be on top
        const z = i + 1;
  ...
  return (
    ...
    >
      <div className="scroll-stack-inner pt-0 px-20 pb-[40vh] min-h-screen">
        {children}
