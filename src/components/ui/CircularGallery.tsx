...
class Title {
  ...
  onResize(planeScaleY: number) {
    if (!this.mesh) return;
    const aspect = this.mesh.scale.x / this.mesh.scale.y;
    const textHeightScaled = planeScaleY * 0.15;
    const textWidthScaled = textHeightScaled * aspect;
    this.mesh.scale.set(textWidthScaled, textHeightScaled, 1);
    this.mesh.position.y = -planeScaleY * 0.5 - textHeightScaled * 0.5 - 0.05;
  }
}
...
class Media {
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    const isMobile = this.screen.width <= 1000;
    const isPortrait = this.screen.height / this.screen.width > 1.35;

    const widthFactor = (isMobile && isPortrait) ? 0.4 : 0.3;
    const heightFactor = (isMobile && isPortrait) ? 0.3 : 0.5;

    this.plane.scale.x = this.viewport.width * widthFactor;
    this.plane.scale.y = this.viewport.height * heightFactor;
    this.plane.program.uniforms.uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y];

    this.bend = (isMobile && isPortrait)
      ? this.originalBend / 3
      : this.originalBend;
    ...
    this.title?.onResize(this.plane.scale.y);
  }
}
...
class App {
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    const dist = Math.hypot(clientX - this.clickStart.x, clientY - this.clickStart.y);
    if (dist < 12 && this.onItemClick) {
      ...
      if (hits.length > 0) {
        const mediaIndex = this.medias.findIndex(m => m.plane === hits[0]);
        this.onItemClick(mediaIndex % (this.mediasImages.length / 2));
      }
    }
  }
